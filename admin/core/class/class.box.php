...
include_once($_SERVER['DOCUMENT_ROOT'].'/admin/core/libs/phpseclib/Net/SSH2.php');
...
function is_valid_box($b_id) {
	$b_info = mysql_query("SELECT * FROM `box` WHERE `boxid` = '$b_id'");

	if (mysql_num_rows($b_info) == 0) {
		$return = false;
	} else {
		$return = true;
	}
...
function valid_box($b_ip) {
	$b_info = mysql_fetch_array(mysql_query("SELECT * FROM `box` WHERE `ip` = '$b_ip'"));
...
function box_name($b_id) {
	$b_info = mysql_fetch_array(mysql_query("SELECT * FROM `box` WHERE `boxid` = '$b_id'"));
...
function box_servers($b_id) {
	$b_info = mysql_query("SELECT * FROM `serveri` WHERE `box_id` = '$b_id'");
...
function box_cache($Box_ID) {
	if (empty($Box_ID)) {
		...
		if ($ssh->login(box_username($Box_ID), box_password($Box_ID))) {
			...
			$previousBoxCache = mysql_num_rows(mysql_query("SELECT `cache` FROM `box` WHERE `boxid` = '$Box_ID'"));
			...
			mysql_query("UPDATE `box` SET `cache` = '".mysql_real_escape_string(gzcompress(serialize($Box_Cache), 2))."' WHERE `boxid` = '".$Box_ID."'");
